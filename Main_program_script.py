...
PROJECT TEAM MEMBERS : Anushka Siri Raghunandan, Ayush Kumar Gupta and Bolisetty Naga Sathya Sai Krishna
PROJECT TITLE: Password Generator
PROJECT DESCRIPTION: A User-friendly GUI window which aims at generating a strong password as per the user's requirement and safely deliver this generated password via E-mail
'''
...
# THE THREE FUNCTIONS USED TO CONFIGURE THE VARIABLE CONSTRAINTS AND ALTER THE PASSWORD GENERATION FORMAT
def uppertoggle():
    global constraints
...
def makstrlab(passwd):
    ...
    if 'WEAK' in pstr:
        strlab.destroy()
        strlab = tkinter.Label(root, text = pstr,fg = 'Red',bg = 'Grey')
        strlab.grid(row=11)
    elif 'FAIR' in pstr:
        strlab.destroy()
        strlab = tkinter.Label(root, text = pstr,fg = 'Yellow3',bg = 'Grey')
        strlab.grid(row=11)
    elif "GOOD" in pstr:
        strlab.destroy()
        strlab = tkinter.Label(root, text = pstr,fg = 'Lawn Green',bg = 'Grey')
        strlab.grid(row=11)
    else:
        strlab.destroy()
        strlab = tkinter.Label(root, text = pstr,fg = 'Green2',bg = 'Grey')
        strlab.grid(row=11)
